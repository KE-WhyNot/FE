...
const SavingsDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        const url = `https://policy.youth-fi.com/api/finproduct/${encodeURIComponent(
          productId
        )}`;
  ...
  useEffect(() => {
    if (product) {
      ...
      const rate =
        selectedRateType === "max" ? product.rates.max : product.rates.base;
          ...
          <div className="tab-content">
            <div className="product-info-grid">
              <div className="info-row">
                ...
                <div className="info-value">{product.details.period}</div>
              </div>
              <div className="info-row">
                <div className="info-label">상품 가이드</div>
                <div
                  className="info-value"
                  dangerouslySetInnerHTML={{ __html: product.details.amount }}
                />
              </div>
              <div className="info-row">
                <div className="info-label">가입방법</div>
                <div className="info-value">{product.details.method}</div>
              </div>
              <div className="info-row">
                <div className="info-label">대상</div>
                <div className="info-value">{product.details.target}</div>
              </div>
              <div className="info-row">
                <div className="info-label">우대조건</div>
                <div
                  className="info-value"
                  dangerouslySetInnerHTML={{
                    __html: product.details.benefitCondition,
                  }}
                />
              </div>
              <div className="info-row">
                <div className="info-label">이자지급</div>
                <div className="info-value">
                  {product.details.interestPayment}
                </div>
              </div>
              <div className="info-row notice">
                <div className="info-label">유의사항</div>
                <div className="info-value">{product.details.notice}</div>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="rate-options">
                <div
                  className={`rate-option ${
                    selectedRateType === "max" ? "active" : ""
                  }`}
                  onClick={() => setSelectedRateType("max")}
                >
                  <span>최고금리</span>
                  <strong>{(product.rates.max * 100).toFixed(2)}%</strong>
                </div>
                <div
                  className={`rate-option ${
                    selectedRateType === "base" ? "active" : ""
                  }`}
                  onClick={() => setSelectedRateType("base")}
                >
                  <span>기본금리</span>
                  <strong>{(product.rates.base * 100).toFixed(2)}%</strong>
                </div>
              </div>
              ...
              <div className="calculation-result">
                <div className="result-row">
                  ...
                  <span className="interest">
                    + {toCurrency(calculated.preTaxInterest)}원
                  </span>
                </div>
                <div className="result-row">
                  <span>세전이자</span>
                ...
                <div className="result-row">
                  <span>이자과세(15.4%)</span>
                  <span className="tax">- {toCurrency(calculated.tax)}원</span>
                </div>
              </div>
