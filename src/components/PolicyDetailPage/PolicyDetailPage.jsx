...
const policyData = [
  { 
    ...
    description: '대구형 청년희망주택 입주자에게 임대보증금 이자를 지원하여 주거부담 경감 및 청년 주거안정을 도모하고자 함.',
    ...
    eligibility: {
      age: '만 19세~만 39세',
      residence: '대구광역시 중구, 동구, 서구, 남구, 북구, 수성구, 달서구, 달성군, 군위군',
      income: '무관',
      education: '제한없음',
      major: '제한없음',
      employmentStatus: '제한없음',
      specializedFields: '제한없음',
      additionalRequirements: '대구형 청년희망주택의 대학생·청년·신혼부부 등 청년계층 입주자 중 전세자금 대출자',
      participationRestrictions: '주거급여수급자 또는 전세금 대출이자·월세 등 정부 및 우리 시 주거지원 받고 있는 대상자',
    },
    application: {
...
const Section = ({ title, data, isOtherSection = false }) => {
    const keyMap = {
        ...
        age: '연령', residence: '거주지역', income: '소득', education: '학력', major: '전공', employmentStatus: '취업상태', specializedFields: '특화분야', additionalRequirements: '추가사항', participationRestrictions: '참여제한 대상',
        procedure: '신청절차', evaluation: '심사 및 발표', site: '신청 사이트', documents: '제출 서류',
        managingOrg: '주관 기관', operatingOrg: '운영 기관', refSite1: '참고사이트 1', refSite2: '참고사이트 2'
    };
                    ...
                    return (
                        <div className="info-row-detail" key={key}>
                            ...
                            <div className="info-value-detail">
                                {key === 'site' ? <a href={value} target="_blank" rel="noopener noreferrer">{value}</a> : value}
                            </div>
                        </div>
                    );
                })}
