...
const AllStocksDetail = () => {
    ...
    const [openingPrice, setOpeningPrice] = useState(0); // ✅ 시가 저장을 위한 상태 추가
    ...
    useEffect(() => {
        const fetchCurrentPrice = async () => {
            try {
                ...
                const current = result?.stckPrpr || 0; // 현재가
                const open = result?.stckOprc || 0;    // 시가 (API 응답 키가 다를 경우 이 부분을 수정하세요)

                setPrice(current);
                setLivePrice(current);
                setOpeningPrice(Number(open)); // 시가 상태 업데이트
            } catch (error) {
    ...
    /** ✅ 실시간 등락률 계산 (시가 기준) - 새로 추가된 부분 */
    useEffect(() => {
        // 현재가와 시가가 모두 유효한 값일 때만 계산
    ...
    }, [livePrice, openingPrice]); // 현재가 또는 시가가 변경될 때마다 재계산
                ...
                <div className="stock-detail-grid">
                    <div className="chart-section widget">
                        ...
                        <div className="chart-info-header detailed">
                            ...
                            {/* ✅ 이 부분이 수정된 계산 로직에 따라 자동으로 업데이트됩니다. */}
