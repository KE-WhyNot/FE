...
const SavingsPage = () => {
  ...
  const [isPeriodAmountFilterOpen, setIsPeriodAmountFilterOpen] =
    useState(false);
  ...
  useEffect(() => {
    const fetchBenefitOptions = async () => {
      ...
      try {
        const benefitRes = await policyAxiosInstance.get(
          `/api/finproduct/filter/special_condition?type=${type}`
        );

        // ✅ 수정된 부분: API 응답이 배열인지, 객체 안의 배열인지 확인 후 설정
        const options = Array.isArray(benefitRes.data)
          ? benefitRes.data
          : benefitRes.data.data;
  ...
  useEffect(() => {
    const fetchDetailedTypeOptions = async () => {
      const type = selectedCategories.includes("적금") ? 2 : 1;
      try {
        const res = await policyAxiosInstance.get(
          `/api/finproduct/filter/special_type?type=${type}`
        );
  ...
  useEffect(() => {
    ...
    const fetchBankOptions = async () => {
      try {
        const response = await policyAxiosInstance.get(
      ...
      try {
        const params = {
          ...
          periods:
            selectedPeriod === "전체"
              ? 0
              : parseInt(selectedPeriod.replace("개월", "")),
          special_conditions:
            selectedBenefits.length > 0 ? selectedBenefits : null,
          special_types:
            selectedProductTypes.length > 0 ? selectedProductTypes : null,
          interest_rate_sort:
            sortOrder === "최고금리순" ? "include_bonus" : "base_only",
        };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        periodAmountFilterRef.current &&
        !periodAmountFilterRef.current.contains(event.target) &&
      ...
      if (
        ...
        !productTypeFilterRef.current.contains(event.target) &&
        !productTypeButtonRef.current.contains(event.target)
      ) {
        setIsProductTypeFilterOpen(false);
      }
      if (
        benefitFilterRef.current &&
        !benefitFilterRef.current.contains(event.target) &&
        !benefitButtonRef.current.contains(event.target)
      ) {
  ...
  const categoryTitle =
    selectedCategories.length === 1 ? selectedCategories[0] : "예·적금";
        ...
        <div className="content-wrapper">
          <div className="filter-bar">
            ...
            <div className="filter-item-container" ref={periodButtonRef}>
              <button
                className={`filter-button ${
                  isPeriodAmountFilterOpen ? "active" : ""
                }`}
                onClick={() =>
                  setIsPeriodAmountFilterOpen(!isPeriodAmountFilterOpen)
                }
              >
                {" "}
                기간·금액 {isPeriodAmountFilterOpen ? (
                  <FaMinus />
                ) : (
                  <FaPlus />
                )}{" "}
                ...
                <div className="filter-panel" ref={periodAmountFilterRef}>
                  ...
                  <div className="panel-actions">
                    ...
                    <button
                      ...
                      onClick={handlePeriodAmountReset}
                    >
                      <FaSyncAlt /> 초기화
                    </button>{" "}
                    <button
                      className="apply-button"
                      onClick={handlePeriodAmountApply}
                    >
                      적용
                    </button>{" "}
                  </div>{" "}
            ...
            <div className="filter-item-container" ref={productTypeButtonRef}>
              <button
                className={`filter-button ${
                  isProductTypeFilterOpen ? "active" : ""
                }`}
                onClick={() =>
                  setIsProductTypeFilterOpen(!isProductTypeFilterOpen)
                }
              >
                {" "}
                상품유형 {isProductTypeFilterOpen ? (
                  <FaMinus />
                ) : (
                  <FaPlus />
                )}{" "}
                ...
                <div className="filter-panel large" ref={productTypeFilterRef}>
                  <div className="panel-section">
                    ...
                    <div className="product-category-button-group">
                      <button
                        className={`period-button ${
                          selectedCategories.includes("예금") ? "active" : ""
                        }`}
                        onClick={() => handleCategoryChange("예금")}
                      >
                        {" "}
                        예금{" "}
                      </button>
                      <button
                        className={`period-button ${
                          selectedCategories.includes("적금") ? "active" : ""
                        }`}
                        onClick={() => handleCategoryChange("적금")}
                      >
                        {" "}
                        적금{" "}
                      </button>
                  ...
                  <div className="panel-section">
                    ...
                    <div className="product-bank-type-button-group">
                      ...
                      <button
                        className={`period-button ${
                          selectedBankCategory === 0 ? "active" : ""
                        }`}
                        onClick={() => {
                          handleBankCategoryChange(0);
                          setSelectedBanks([]);
                        }}
                      >
                        전체
                  ...
                  <div className="panel-actions">
                    <button
                      className="reset-button"
                      onClick={handleProductTypeReset}
                    >
                      <FaSyncAlt /> 초기화
                    </button>
                    <button
                      className="apply-button"
                      onClick={handleProductTypeApply}
                    >
                      적용
                    </button>
                  </div>
            ...
            <div className="filter-item-container" ref={benefitButtonRef}>
              ...
              {isBenefitFilterOpen && (
                <div className="filter-panel large" ref={benefitFilterRef}>
                      ...
                      {benefitOptions.map((benefit) => {
                        ...
                        return (
                          <button
                            key={benefit.id}
                            className={`period-button ${
                              selectedBenefits.includes(buttonText)
                                ? "active"
                                : ""
                            }`}
                  ...
                  <small className="panel-disclaimer">
                    *신협 상품에는 적용되지 않습니다.
                  </small>{" "}
                  <div className="panel-actions">
                    {" "}
          ...
          <div className="list-info-bar">
            <span className="total-count">
              {" "}
              총 <strong>{totalCount}</strong>건의 {categoryTitle} 정보가
              있습니다.{" "}
            </span>
          ...
          <div className="savings-list-container">
            {savingsData.map((item) => (
              <Link
                to={`/savings/${item.finproduct_id}`}
                key={item.finproduct_id}
                className="savings-item-link"
              >
                <div className="savings-item">
                  <div className="item-left">
                    ...
                    <div className="product-info">
                      ...
                      <div className="tags">
                        {item.product_type_chip.map((tag, i) => (
                          <span
                            key={i}
                            className={`tag ${
                              tag === "특판" ? "highlight" : ""
                  ...
                  <div className="item-right">
                    <div className="rate-info">
                      <span className="rate-max">
                        최고 {item.max_interest_rate}%
                      </span>
                      <span className="rate-base">
                        기본 {item.min_interest_rate}%
                      </span>
                    </div>
                  </div>
                </div>
        ...
        >
          <div className="bank-modal-body">
            <div className="bank-modal-tabs">
              <button
                className={`bank-modal-tab ${
                  modalBankType === 1 ? "active" : ""
                }`}
              ...
              <label>
                <input
                  ...
                  checked={
                    ...
                    tempSelectedBanks.length === bankOptions.length
                  }
                />
                모든 은행 선택
              </label>
            ...
            <div className="bank-grid">
              {bankOptions.map((bank) => (
                <div
                  key={bank.id}
                  className={`bank-item ${
                    tempSelectedBanks.includes(bank.nickname) ? "active" : ""
                  }`}
            ...
            <div className="bank-modal-footer">
              <button
                ...
                onClick={handleApplyBankSelection}
              >
                확인
              </button>
            </div>
